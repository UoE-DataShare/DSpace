...
public class AuthorizeServiceImpl implements AuthorizeService
{
	...
	public void authorizeAction(Context c, EPerson e, DSpaceObject o, int action, boolean useInheritance) throws AuthorizeException, SQLException
	{
		// DATASHARE - start
		if (e != null) {
			log.debug("EPerson Email: " + e.getEmail());
			log.debug("EPerson ID: " + e.getID());
			log.debug("EPerson NetId: " + e.getNetid());
			log.debug("EPerson Groups:");
			for(Group group : e.getGroups()) {
			...
			if (o != null) {
				...
				for(ResourcePolicy rp : o.getResourcePolicies()) {
					...
					log.debug("EPerson Group: " + rp.getGroup());
					if (rp.getGroup() != null) {
						log.debug("EPerson Group: " + rp.getGroup().getName());
					}	
				}
			}
			
		} else {
			log.debug("EPerson is null");
		}
		// DATASHARE - end

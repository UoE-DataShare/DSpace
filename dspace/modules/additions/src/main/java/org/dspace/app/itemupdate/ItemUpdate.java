...
public class ItemUpdate {
	...
	// DATASHARE - start
	private static Logger log = Logger.getLogger(ItemUpdate.class);
	// DATASHARE - end
...
//special run states        
        ...
        for (String dirname : dircontents)
        {
    		...
    		try
    		{
	    		...
	    		if (!isTest)
	    		{
					...
					for (UpdateAction action : actionMgr) {
						pr("Datashare action: " + action);
						pr("Datashare UpdateBitstreamsAction.class.isInstance(action): " + UpdateBitstreamsAction.class.isInstance(action));
						// Delete zip files only if bitstreams are added or deleted
						// The cron job will create a new zip file overnight
						if (UpdateBitstreamsAction.class.isInstance(action)) {
							deleteItemDatasetZipFile(item);
						}
					}
				    // DATASHARE - end	  
	    		}
    ...
    // DATASHARE - start
    private void deleteItemDatasetZipFile(Item item) {
    	try {
    		new ItemDataset(item).delete();
    	} catch(Exception e) {
    		pr("Failed to delete Item Dataset zip file" + e);

...
//				Context context = ContextUtil.obtainContext(request);
				...
				try
				{
					...
					// To fix loss of location in Header after ease-login
					if(!response.containsHeader(LOCATION) || url.endsWith("/ease-login")) {
						
						LOG.info("Adding 'Location' header: "  + dspaceUrl);
						response.addHeader(LOCATION, dspaceUrl);
					}
					...
					if(interruptUrl == null){
						...
						if(StringUtils.isEmpty(interruptUrl)) {
							interruptUrl = dspaceUrl;
						}
